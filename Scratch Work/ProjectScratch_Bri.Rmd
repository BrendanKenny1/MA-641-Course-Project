...
```{r}
# Fit ARMA model
arma_model1 <- Arima(nonseasonal_ts, order=c(1,0,1))
summary(arma_model1)
```
...
```{r}
# ARMA(2,1) Model
arma_model2 <- Arima(nonseasonal_ts, order=c(2,0,1))
summary(arma_model2)
```
...
```{r}
# Fit SARIMA model
sarima_model <- auto.arima(nonseasonal_ts)
summary(sarima_model)
```
...
returns <- diff(log(nonseasonal_ts1))
returns <- returns[!is.na(returns)]
plot(returns, main="Weekly Apple Stock Prices", ylab="Close Price", xlab="Time")
```
