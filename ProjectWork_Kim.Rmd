...
adjusted_arima_model <- Arima(combined_diff_data, order = c(0, 0, 2), seasonal = c(0, 0, 1))
...
adjusted_arima_model_2 <- Arima(combined_diff_data, order = c(0, 0, 2), seasonal = c(0, 0, 2))
...
```
```{r}
# Create an xts object from combined_diff_data
combined_xts <- xts(combined_diff_data, order.by = as.Date(time(combined_diff_data)))
...
# Extract and print the AIC of the GARCH model
garch_aic <- infocriteria(garch_fit)["Akaike"]
print(paste("AIC of the GARCH model:", garch_aic))
